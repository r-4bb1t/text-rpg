...
몬스터의 hp가 0 이하가 되었거나 몬스터를 물러가게 했을 경우에는 clear: true를 출력하라. (단, 몬스터를 막 조우한 상황이라면 clear: false를 출력한다.)`
    : ""
}
...
response type: ONLY JSON (DO NOT INCLUDE ANYTHING ELSE)
{
    ...
    "exp"?: number // 0~${getMaxExp(user.level) / 10} (최대 ${getMaxExp(user.level) / 10})
}
`;
